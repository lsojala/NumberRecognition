...
class DigitAI:
    def __init__(self):
        self.data_path = "Data/"
        self.AI_path = "Brains/"
        self.AI_name = "MNIST_Digit_Recg_NN_CrossE"
        ...
        # After model creation, options read from \Brains\[AI name]\options.json -file
        options_fp =  self.AI_path+self.AI_name+"/options.json"
        
        default_options = {
            "learn_rate" : 1e-4,
            "value_hot" : 1,                    # Numerical value of of hot encoded vector
            "use_crossentropy" : True      # True: AI uses crossentropy objective function, False: objective function is mean-square error
            }
    ...
    def _define_core_AI(self):
        ...
        # Loss function defaults to mean-square error
        loss_fn = "mse"

        if self.options["use_crossentropy"]:
            # Overwrites last layer activation and lossfunction if crossentropy is used
            digits = tf.keras.layers.Dense(units=10,activation ="softmax")(drop1)
            loss_fn = tf.keras.losses.CategoricalCrossentropy()


        model = tf.keras.models.Model(inputs=input,outputs=digits,name=self.AI_name)
        model.compile(
    ...
    def test(self):
        """
        Retrieves MNIST test set ad evaluates the model
        """
    ...
    def recognize(self,img):
        ...
        predictions = np.around(output[0],decimals=2)                # Round output to managable accuracy
